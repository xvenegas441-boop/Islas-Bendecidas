...
export const data = new SlashCommandBuilder()
  .setName('listargenshin')
  .setDescription('🌟 Lista de personajes de Genshin Impact');

const LOGO_URL =
  'https://static.wikia.nocookie.net/gensin-impact/images/2/2b/Genshin_Impact_logo.png';

const makeEmbed = (content, pageIndex, totalPages, stats) => {
  const embed = new EmbedBuilder()
    .setAuthor({
      name: '🌟 Genshin Impact - Lista de Personajes'
    })
    .setColor('#4ADEDE') // Genshin teal color
    .setDescription(content)
    .setFooter({
      text: `Página ${pageIndex + 1} de ${totalPages} • ${stats.total} personajes totales`
    })
    .setTimestamp();
...
const makeRow = (pageIndex, totalPages) =>
  new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId('genshin_prev')
      .setLabel('⬅️ Anterior')
      .setStyle(ButtonStyle.Primary)
      .setDisabled(pageIndex === 0),
    new ButtonBuilder()
      .setCustomId('genshin_next')
...
export async function execute(interaction) {
  try {
    ...
    const res = await pool.query(
      'SELECT name, claimed_by FROM genshin_characters ORDER BY name ASC'
    );

    if (res.rowCount === 0) {
      await interaction.editReply({ content: 'No hay personajes registrados en la base de datos.' });
      return;
    }
    ...
    collector.on('collect', async i => {
      try {
        ...
        if (i.customId === 'genshin_prev') {
          currentPage = Math.max(0, currentPage - 1);
        } else if (i.customId === 'genshin_next') {
    ...
    console.error('Error in /listargenshin command:', error);
    if (interaction.deferred || interaction.replied) {
      await interaction.followUp({ content: '❌ No se pudo obtener la lista de personajes.', flags: 64 }).catch(() => {});
    } else {
      await interaction.reply({ content: '❌ No se pudo obtener la lista de personajes.', flags: 64 }).catch(() => {});
    }
  }
}